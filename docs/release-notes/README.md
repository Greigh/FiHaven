...
| [1.6.2](v1.6.2/ios53-android53.md) | iOS 53 · Android 53 | 2026-09-02 | A checking/savings/investment row can be pinned to a specific bank account from the editor — the picker cards already had — so a sync can find it and its balance reaches the Balances tab as an Accept/Decline; the post-sync prompt now covers account balances too. Android rolls up the Compose/Firebase/Plaid/JUnit/Gradle bumps merged since 52. **No server deploy, no migration, no sign-out** |
