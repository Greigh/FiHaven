...
import { openProDialog } from './pro.js';
  ...
  var ICONS = {
    ...
    networth:  '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M12 3v18"/><path d="M5 7h14"/><path d="M5 7l-3 6h6z"/><path d="M19 7l-3 6h6z"/><path d="M8 21h8"/></svg>',
    balances:  '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M3 6.5C3 5.1 7 4 12 4s9 1.1 9 2.5S17 9 12 9 3 7.9 3 6.5z"/><path d="M3 6.5v11C3 18.9 7 20 12 20s9-1.1 9-2.5v-11"/><path d="M3 12c0 1.4 4 2.5 9 2.5s9-1.1 9-2.5"/></svg>',
  ...
  var MORE_TABS = ['subscriptions', 'calendar', 'history', 'payoff', 'rewards', 'networth', 'balances'];
  var TAB_LABELS = {
    ...
    networth: 'Net Worth', balances: 'Balances',
  };
  // The More menu is a full-width list, so it can afford the unambiguous name.
  // In the tab strip the short one wins — "Balances" next to "Net Worth" reads
  // fine, where "Account Balances" would wrap.
  var TAB_MORE_LABELS = {
    balances: 'Account Balances',
  };
  function moreLabel(name) { return TAB_MORE_LABELS[name] || TAB_LABELS[name]; }
  ...
  function buildDashboard(host) {
    ...
    var moreItems = MORE_TABS.map(function (name) {
      return tab(name, moreLabel(name), {
