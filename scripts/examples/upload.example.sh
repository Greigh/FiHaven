...
build_production_env() {
  ...
  TMP_ENV=$(mktemp)
  {
    ...
    grep -E '^(PORT|TURNSTILE_SECRET|TURNSTILE_SITEKEY|SESSION_COOKIE|SESSION_TTL_HOURS|TOKEN_TTL_DAYS|SMTP_HOST|SMTP_PORT|SMTP_USER|SMTP_PASS|MAIL_FROM|MAIL_CHECK_TO|MFA_ENCRYPTION_KEY|STRIPE_[A-Z_]+|GOOGLE_OAUTH_CLIENT_ID|APPLE_CLIENT_ID|OAUTH_VERIFY_MODE|APPLE_VERIFY_ENABLED|PUBLIC_ORIGIN|ADMIN_EMAILS|PASSKEY_[A-Z_]+|IAP_[A-Z_]+|GOOGLE_VERIFY_ENABLED|GOOGLE_PLAY_[A-Z_]+|APNS_[A-Z_]+|FCM_[A-Z_]+|VAPID_[A-Z_]+|HOUSEHOLD_MAX_[A-Z_]+|SUBSCRIPTION_LINK_INBOX)=' .env || true
