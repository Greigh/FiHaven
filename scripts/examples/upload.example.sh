...
build_production_env() {
  ...
  TMP_ENV=$(mktemp)
  {
    ...
    grep -E '^(PORT|TURNSTILE_SECRET|TURNSTILE_SITEKEY|SESSION_COOKIE|SESSION_TTL_HOURS|SMTP_HOST|SMTP_PORT|SMTP_USER|SMTP_PASS|MAIL_FROM|MAIL_CHECK_TO|MFA_ENCRYPTION_KEY|STRIPE_[A-Z_]+|GOOGLE_OAUTH_CLIENT_ID|APPLE_CLIENT_ID|OAUTH_VERIFY_MODE|PUBLIC_ORIGIN|ADMIN_EMAILS|PASSKEY_[A-Z_]+)=' .env || true
