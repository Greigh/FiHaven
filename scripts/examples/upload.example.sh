...
build_production_env() {
  ...
  TMP_ENV=$(mktemp)
  {
    ...
    grep -E '^(PORT|TURNSTILE_SECRET|TURNSTILE_SITEKEY|SESSION_COOKIE|SESSION_TTL_HOURS|SMTP_HOST|SMTP_PORT|SMTP_USER|SMTP_PASS|MAIL_FROM|MFA_ENCRYPTION_KEY|STRIPE_[A-Z_]+|PUBLIC_ORIGIN|ADMIN_EMAILS)=' .env || true
  } > "$TMP_ENV"
